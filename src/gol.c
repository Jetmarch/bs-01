...
int main(void)
{
    ...
    char *golLogicCode = LoadFileText("resources/shaders/gl430/gol_logic.glsl");
    ...
    Shader golRenderShader = LoadShader(NULL, "resources/shaders/gl430/gol_render.glsl");
    int resUniformLoc = GetShaderLocation(golRenderShader, "resolution");

    // Game of Life transfert shader (CPU<->GPU download and upload)
    char *golTransfertCode = LoadFileText("resources/shaders/gl430/gol_transfert.glsl");
